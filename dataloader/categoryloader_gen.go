...
type CategoryLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*model.Category, []error)
...
type CategoryLoader struct {
	...
	cache map[string]*model.Category
...
func (l *CategoryLoader) Load(key string) (*model.Category, error) {
...
func (l *CategoryLoader) LoadThunk(key string) func() (*model.Category, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Category, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CategoryLoader) LoadAll(keys []string) ([]*model.Category, []error) {
	results := make([]func() (*model.Category, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	categorys := make([]*model.Category, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CategoryLoader) LoadAllThunk(keys []string) func() ([]*model.Category, []error) {
	results := make([]func() (*model.Category, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CategoryLoader) Prime(key string, value *model.Category) bool {
	l.mu.Lock()
	var found bool
...
func (l *CategoryLoader) unsafeSet(key string, value *model.Category) {
	if l.cache == nil {
		l.cache = map[string]*model.Category{}
	}
	l.cache[key] = value
}
